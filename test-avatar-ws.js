...
(async () => {
  ...
  // ═══ 2. une vraie partie (3 manches, le minimum) : mise en situation, « à toi », résultats
  a.send({ action: 'start', rounds: 3 });
  for (let i = 0; i < 3; i++) {
    const round = await b.wait(type('round'));
    if (i === 0) t('en jeu (manche) : PP de A, emoji de B', same(find(round.players, ja.id).avatar, IMG_A) && same(find(round.players, jb.id).avatar, EMO_B));
    const go = await b.wait(type('go'), 8000);
    if (i === 0) t('en jeu (« à toi ») : PP de A retransmise', same(find(go.players, ja.id).avatar, IMG_A));
    const legal = Object.keys(round.scene.options).find((k) => round.scene.options[k].legal !== false);
    a.send({ action: 'answer', passId: legal });
    b.send({ action: 'answer', passId: legal });
    const res = await b.wait(type('results'));
    if (i === 0) t('résultats de manche : PP de A, emoji de B', same(find(res.results, ja.id).avatar, IMG_A) && same(find(res.results, jb.id).avatar, EMO_B));
    a.send({ action: 'next' });
  }
